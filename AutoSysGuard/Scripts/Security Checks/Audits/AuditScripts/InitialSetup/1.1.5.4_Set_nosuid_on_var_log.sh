...
# Function to check if nosuid option is set for /var/log
check_nosuid() {
    echo "Checking if the 'nosuid' option is set for /var/log..."
    if findmnt -kn /var/log | grep -q 'nosuid'; then
        echo "'nosuid' option is set for /var/log."
        return 0  # No action needed
    else
        echo "'nosuid' option is NOT set for /var/log."
...
apply_remediation() {
    ...
    if mount | grep -q '/var/log'; then
        ...
        if grep -q '/var/log' /etc/fstab; then
            # Use sed to add nosuid to the mounting options
            sed -i.bak "s/\(\/var\/log.*\)\(defaults.*\)/\1\2,nosuid/" /etc/fstab
        ...
        echo "/var/log remounted with 'nosuid' option."
    else
...
check_nosuid
if [ $? -eq 1 ]; then
    read -p "Do you want to apply remediation to set the 'nosuid' option for /var/log? (y/n): " response
    case "$response" in
        [yY][eE][sS]|[yY])
            apply_remediation
            ;;
