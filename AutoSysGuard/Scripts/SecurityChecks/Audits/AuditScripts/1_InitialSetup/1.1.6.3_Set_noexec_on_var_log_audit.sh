...
check_noexec() {
    echo "Checking noexec option for /var/log/audit..."
    if findmnt -kn /var/log/audit | grep -qv 'noexec'; then
        echo "The noexec option is NOT set for /var/log/audit."
        return 1  # Return 1 if noexec is not set
    else
        echo "The noexec option is set for /var/log/audit."
...
apply_remediation() {
    ...
    if mount | grep -q '/var/log/audit'; then
        ...
        if grep -q '/var/log/audit' /etc/fstab; then
            sed -i 's/\(\/var\/log\/audit.*\)\(defaults\)/\1\2,noexec/' /etc/fstab
        else
        ...
        echo "Remounted /var/log/audit with noexec option."
    else
