...
if [ -z "$l_output2" ]; then
    ...
    if [[ "$apply_remediation" == "y" || "$apply_remediation" == "Y" ]]; then
        ...
        if grep -q "^net.ipv4.tcp_syncookies" "$config_file"; then
            sed -i "s/^net.ipv4.tcp_syncookies=.*/net.ipv4.tcp_syncookies=1/" "$config_file"
        else
            echo "net.ipv4.tcp_syncookies=1" >> "$config_file"
        fi
