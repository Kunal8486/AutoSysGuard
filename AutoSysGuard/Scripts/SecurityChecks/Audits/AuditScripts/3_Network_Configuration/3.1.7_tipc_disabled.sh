...
# Audit and remediation script for the "tipc" module
...
if [ -z "$l_output2" ]; then
    ...
    # Ask user if they want to apply remediation
    read -p "Do you want to apply remediation for the module \"$l_mname\"? (y/n): " response
    if [[ "$response" =~ ^[Yy]$ ]]; then
